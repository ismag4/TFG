...
def distancia_comovil(p, c, z):
    ...
    for z in z[0]:
                #w0 = p[3] y wa = p[4]
        intg, error = integrate.quad(lambda x: 1/np.sqrt(p[1]*(1+x)**3+(1-p[1])*(1+x)**(3*(1+p[3]))*np.exp(3*p[4]*x)), 0, z)      
...
#a lo mejor hace falta prior en w00
...
w0 = -1.0 #parametro de ecuación de estado
wa = 0
#recogemos dichas variables en un array
p = np.array([h,omega_m, M, w0, wa])
...
w0_media = np.mean(samples[:,3])
w0_er = np.std(samples[:,3])
print(w0_media,"+-", w0_er)

wa_media = np.mean(samples[:,4])
wa_er = np.std(samples[:,4])
print(wa_media,"+-", wa_er)
